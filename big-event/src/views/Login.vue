<script setup>
import { User, Lock } from '@element-plus/icons-vue'
import { ref } from 'vue'
import { ElMessage, roleTypes } from 'element-plus'
//控制注册与登录表单的显示， 默认显示注册
const isRegister = ref(false)
//定义数据模型
const registerData = ref({
    username: '',
    password: '',
    rePassword: '',
})

//校验密码函数
const checkRePassword = (value, callback) => {
    if (value == '') {
        callback(new Error('请再次确认密码'))
    } else if (value !== registerData.value.password) {
        callback(new Error('两次密码不一致'))
    } else {
        /* 什么都没传代表参数校验通过 */
        callback()
    }
}

//定义表单校验规则  可以参考element 官网
const rules = {
    username: [
        /* 用户名不能为空   required:true  表示必填项  message:提示信息  trigger: blur 表示失去焦点时触发校验*/
        { required: true, message: '请输入用户名', trigger: 'blur' },
        { min: '5', max: '16', message: '长度为5到16为非空字符', trigger: 'blur' }
    ],
    password: [
        { required: true, message: '请输入密码', trigger: 'blur' },
        { min: '5', max: '16', message: '长度为5到16为非空字符', trigger: 'blur' }
    ],
    rePassword: [
        { validator: checkRePassword, trigger: 'blur' }
    ]
}


//导入注册接口
import { userRegisterService } from '@/api/user'
//调用后台接口完成注册
const register = async () => {
    //调用注册接口 当返回的http状态码为0时，提示注册成功
    let result = await userRegisterService(registerData.value);
    /* if (result.code === 0) {
         弹窗提示
        alert(result.msg ? result.msg : '注册成功');
    } else {
        /* 弹窗提示 
        alert('注册失败');
    } */    
    ElMessage.success(result.msg ? result.msg : '注册成功');
}

//导入登录接口
import { userLoginService } from '@/api/user'

//登录页面复用 注册页面的数据模型
//校验规则复用注册页面的校验规则
//提供登录函数
//导入vue-router的组件 useRouter
import { useRouter } from 'vue-router'
const router = useRouter();
//导入token模块
import { useTokenStore } from '@/stores/token.js'
//调用token模块的函数
const tokenStore =useTokenStore();
const login = async () => {
    //调用接口完成登录
    let result = await userLoginService(registerData.value);
    /*  if(result.code === 0){
         /* 弹窗提示 
         alert(result.msg ? result.msg : '登录成功');
     } else {   
         /* 弹窗提示 
         alert('登录失败');
     } */
    ElMessage.success(result.msg ? result.msg : '登录成功');
    //登录成功后把后端发来的token响应给tokenStore ：把得到的token存储到pinia中
    tokenStore.setToken(result.data);
    //登录成功后，跳转到首页
    router.push('/')
}

//定义函数用于清空数据模型
const clearRegisterData = () => {
    registerData.value = {
        username: '',
        password: '',
        rePassword: ''
    }
}

</script>

<template>
    <el-row class="login-page">
        <el-col :span="12" class="bg"></el-col>
        <el-col :span="6" :offset="3" class="form">
            <!-- 注册表单 -->
            <!-- 通过 model 给表达绑定数据模型   在el-form-item 中使用 v-model 绑定数据模型的属性           ：rules 添加校验规则-->
            <el-form ref="form" size="large" autocomplete="off" v-if="isRegister" :model="registerData" :rules="rules">
                <el-form-item>
                    <h1>注册</h1>
                </el-form-item>
                <!-- prop:  绑定校验规则的属性名 -->
                <el-form-item prop="username">
                    <el-input :prefix-icon="User" placeholder="请输入用户名" v-model="registerData.username"></el-input>
                </el-form-item>
                <el-form-item prop="password">
                    <el-input :prefix-icon="Lock" type="password" placeholder="请输入密码"
                        v-model="registerData.password"></el-input>
                </el-form-item>
                <el-form-item prop="rePassword">
                    <el-input :prefix-icon="Lock" type="password" placeholder="请再次输入密码"
                        v-model="registerData.rePassword"></el-input>
                </el-form-item>
                <!-- 注册按钮 -->
                <el-form-item>
                    <el-button class="button" type="primary" auto-insert-space v-on:click="register">
                        注册
                    </el-button>
                </el-form-item>
                <el-form-item class="flex">
                    <el-link type="info" :underline="'never'" @click="isRegister = false; clearRegisterData()">
                        ← 返回
                    </el-link>
                </el-form-item>
            </el-form>
            <!-- 登录表单 -->
            <el-form ref="form" size="large" autocomplete="off" v-else :model="registerData" :rules="rules">
                <el-form-item>
                    <h1>登录</h1>
                </el-form-item>
                <el-form-item prop="username">
                    <el-input :prefix-icon="User" placeholder="请输入用户名" v-model="registerData.username"></el-input>
                </el-form-item>
                <el-form-item prop="password">
                    <el-input name="password" :prefix-icon="Lock" type="password" placeholder="请输入密码"
                        v-model="registerData.password"></el-input>
                </el-form-item>
                <el-form-item class="flex">
                    <div class="flex">
                        <el-checkbox>记住我</el-checkbox>
                        <el-link type="primary" :underline="'never'">忘记密码？</el-link>
                    </div>
                </el-form-item>
                <!-- 登录按钮 -->
                <el-form-item>
                    <el-button class="button" type="primary" auto-insert-space @click="login">登录</el-button>
                </el-form-item>
                <el-form-item class="flex">
                    <el-link type="info" :underline="'never'" @click="isRegister = true; clearRegisterData()">
                        注册 →
                    </el-link>
                </el-form-item>
            </el-form>
        </el-col>
    </el-row>
</template>

<style lang="scss" scoped>
/* 样式 */
.login-page {
    height: 100vh;
    background-color: #fff;

    .bg {
        background: url('@/assets/logo2.png') no-repeat 60% center / 240px auto,
            url('@/assets/login_bg.jpg') no-repeat center / cover;
        border-radius: 0 20px 20px 0;
    }

    .form {
        display: flex;
        flex-direction: column;
        justify-content: center;
        user-select: none;

        .title {
            margin: 0 auto;
        }

        .button {
            width: 100%;
        }

        .flex {
            width: 100%;
            display: flex;
            justify-content: space-between;
        }
    }
}
</style>